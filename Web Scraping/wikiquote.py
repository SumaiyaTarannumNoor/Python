...
def scrape_wikiquotes():
    url = "https://en.wikiquote.org/wiki/Proverbs"
    ...
    for li in soup.select("div.mw-parser-output > ul > li"):
        ...
        if len(text) >10 and not text.startswith("Retrieved"):
            quotes.append(text) 

    with open("wikiquotes.txt", "w", encoding='utf-8') as f:
        for quote in quotes:
            f.write(quote + '\n')   

    print(f"Wikiquotes: {len(quotes)} saved in wikiquotes.txt.")
